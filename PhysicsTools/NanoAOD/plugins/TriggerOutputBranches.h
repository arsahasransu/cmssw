...
public:
  TriggerOutputBranches(const edm::BranchDescription *desc, const edm::EDGetToken &token, bool isPFScouting)
      : m_token(token), m_lastRun(-1), m_fills(0), m_processName(desc->processName()), m_isPFScouting(isPFScouting) {
